...
import 'package:aladdin_franchise/src/features/dialogs/coupon/detail_coupon.dart';
...
class CouponList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final numberOfAdults = ref.watch(checkoutProvider.select((value) => value.numberOfAdults));
    if (coupons.isEmpty) {
      return EmptyDataWidget(
        message: S.current.notDiscountApply,
      );
    }
                        ...
                        showConfirmAction(
                          ...
                          action: () async {
                            ...
                            if (!DevConfig.useCoupon) {
                              ...
                              result =
                                  await ref.read(checkoutProvider.notifier).removeCoupon(coupon);
                            }
                            ...
                            children: discountView.map((e) {
                              ...
                              if (DevConfig.useCoupon) {
                                if (e.type == DiscountTypeEnum.percent.key) {
                                  amountView = AppUtils.getPercentValue(e.amount) ?? '';
                                } else {
                                ...
                                if (coupon.type == null) {
                                  ...
                                  //                 xQuantity += (AppUtils.convertToDouble(cbi.quantity)
                                  //                         ?.toInt() ??
                                  //                     0);
                                  //               }
                                  //             }
