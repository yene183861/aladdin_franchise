  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            TitleLineWidget(title: S.current.server_status),
            const Gap(12),
            Consumer(
              builder: (context, ref, child) {
                var realtimeStatus = ref.watch(
                    homeProvider.select((value) => value.realtimeStatus));
                return Container(
                  padding:
                    ...
                    if (!active) {
                      showConfirmAction(
                        ...
                        action: () async {
                          ...
                          await LocalStorage.setCustomerLanguageLocal(
                              AppLanguageLocal.vietnamese);
                          ref
                              .read(homeProvider.notifier)
                              .onChangeCustomerLanguage();
                        },
                      );
                    } else {
                      showDoneSnackBar(
                          context: context,
                          message:
                              '${S.current.current_language_use} [${S.current.tiengViet}]');
                    }
                  },
                ),
                _ChipLanguageLocalWidget(
    ...
    return ChoiceChip(
      ...
      onSelected: (value) async {
        ...
        if (!active) {
          ...
          showDoneSnackBar(
              context: context,
              message: '${S.current.current_language_use} [$title]');
        }
      },
    );
  }
