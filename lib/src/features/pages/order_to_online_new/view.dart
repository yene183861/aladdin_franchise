...
class NewOrderToOnlinePage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _NewOrderToOnlinePageState();
}
...
class _NewOrderToOnlinePageState extends ConsumerState<NewOrderToOnlinePage> {
  _listenEvent(BuildContext context, WidgetRef ref) =>
      (NewOrderToOnlineEvent? previous, NewOrderToOnlineEvent? next) {
        final state = ref.read(orderToOnlinePageProvider);
        switch (next) {
          case NewOrderToOnlineEvent.loading:
            showProcessingDialog(
              context,
              message: state.message,
            );
            break;
          case NewOrderToOnlineEvent.success:
            Navigator.pop(context);
            break;
          case NewOrderToOnlineEvent.error:
            Navigator.pop(context);
            showMessageDialog(
              context,
              message: state.message,
  ...
  Widget build(BuildContext context) {
    ref.listen<NewOrderToOnlineEvent>(
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(S.current.order_to_online),
      ...
      body: Stack(
        children: [
              ...
              WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                ...
                if (data.isNotEmpty) {
                  var check = orderIdSelect == null
                      ? null
                      : data.keys.toList().firstWhereOrNull((e) => e == orderIdSelect);
          ...
          Consumer(builder: (context, ref, child) {
            var showChat = ref.watch(orderToOnlinePageProvider.select((value) => value.showChat));
          ...
          Positioned(
            ...
            child: ChatAnchorButton(
              hasUnread: false,
              onTap: () {
                ref.read(orderToOnlinePageProvider.notifier).onChangeShowChat(null);
              },
            ),
          ),
