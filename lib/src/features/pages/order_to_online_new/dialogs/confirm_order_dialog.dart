            ...
            children: [
                    ...
                    Expanded(child: Consumer(
                      builder: (context, ref, child) {
                        ...
                        return orderToOnline.when(
                          skipError: false,
                          data: (data) {
                            var listItems = (data[orderIdSelect]?['items'] ??
                                []) as List<RequestOrderModel>;
                            var dataView = listItems.firstWhereOrNull(
                                (e) => e.id == requestSelect?.id);
              ...
              if (!useKds) ...[
                ...
                Consumer(builder: (context, ref, child) {
                  var orderIdSelect = ref.watch(orderToOnlinePageProvider
                      .select((value) => value.orderIdSelect));

                  Map<int?, List<ProductModel>> productPrint = ref
                      .read(menuProvider.notifier)
                      .mapO2oItemWithPrintType(requestSelect?.listItem ?? []);
                  Set<int> printerType =
                      productPrint.keys.whereType<int>().toSet();
                  ...
                  return PrinterSelectorForOrderAction(
                    ...
                    defaultPrinterProvider:
                        o2oPrinterByOrderProvider(orderIdSelect),
...
class __ProductLineState extends ConsumerState<_ProductLine> {
  @override
  Widget build(BuildContext context) {
    var listItem = ref.watch(orderToOnlinePageProvider
