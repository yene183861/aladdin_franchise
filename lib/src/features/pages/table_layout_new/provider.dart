...
class TableLayoutPageNotifier extends StateNotifier<TableLayoutPageState> {
  ...
  void onChangeTableSelect(
    ...
    bool select = true,
    bool serving = false,
    List<TableModel> using = const [],
    List<OrderModel> orders = const [],
  }) {
    var data = Set<TableModel>.from(state.tableSelect);
    if (select) {
      if (serving) {
        data.removeWhere((e) => using.contains(e));
        // k phải đơn bàn khôi phục lại
        var order =
            orders.firstWhereOrNull((e) => e.getTableIds.contains(table.id) && e.isRestore == 0);
        if (order != null) {
          data.addAll(using.where((e) => order.getTableIds.contains(e.id)));
          data.removeWhere(
            (e) => !order.getTableIds.contains(e.id),
          );
        } else {
          data.add(table);
        }
    ...
    state = state.copyWith(tableSelect: data);
  }
}
