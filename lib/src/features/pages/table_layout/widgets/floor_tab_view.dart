  ...
  await showDialog(
    ...
    builder: (_) {
      return Consumer(builder: (context, ref, child) {
        return AlertDialog(
          title: Text(S.current.floor_management),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(S.current.floor_name),
                if (kDebugMode) Text(item.id),
                const Gap(8),
                AppTextFormField(
                  ...
                  children: [
                    ...
                    AppButton(
                      ...
                      onPressed: () async {
                        ...
                        ref.read(tableLayoutPageProvider.notifier).updateFloor(
                              floorId: item.id,
                              delete: false,
                              name: name,
                            );
...
Future<bool?> _showDeleteConfirmDialog(BuildContext context, String floorName) {
  return showDialog<bool?>(
    ...
    builder: (_) => AlertDialog(
      ...
      title: Text(S.current.confirm_deletion),
      content: Text(S.current.confirm_deletion_msg(floorName)),
      actions: [
      ...
      return Consumer(
        builder: (context, ref, child) {
          return AlertDialog(
            ...
            title: Text(S.current.add_floor),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                AppTextFormField(
                  textController: controller,
                  hintText: S.current.floor_name_example,
                  autofocus: true,
                ),
              ],
            ),
            actions: [
              ...
              AppButton(
                ...
                onPressed: () async {
                  final name = controller.text.trim();
                  if (name.isEmpty) return;
                  await ref.read(tableLayoutPageProvider.notifier).addFloor(name);
