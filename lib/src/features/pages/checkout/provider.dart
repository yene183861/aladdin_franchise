...
class CheckoutPageNotifier extends StateNotifier<CheckoutPageState> {
  ...
  Future<bool> onUpdateOrderInvoice(
    ...
    updateEvent(
        isUpdate ? CheckoutEvent.updateInvoice : CheckoutEvent.insertInvoice);
    try {
      var order = getOrderSelect();
      if (order == null) throw S.current.noOrderSelect;
  ...
  Future<FindCustomerStatus> findCustomer(
    ...
    try {
      ...
      do {
        try {
          result = await _customerRepository.findCustomer(
              phoneNumber: phone, order: order);
          break;
        } catch (ex) {
          countRetry++;
      ...
      state = state.copyWith(
          message: "${S.current.find_customer_not_found} ($phone)");
    ...
    try {
      ...
      while (retryTempPayment < timesTempPayment) {
        try {
          ...
          if (printers.isEmpty) {
            final resultPrinter =
                await _orderRepository.getPrinterBill(order, [1]);
  ...
  void loadPaymentMethods() async {
    try {
      ...
      if (order == null) throw S.current.noOrderSelect;
      final result =
          await _restaurantRepository.getPaymentMethod(orderId: order.id);
  ...
  Future<void> loadBanksInfo() async {
    try {
      ...
      final apiBankParam = ApiBankParam(
          state.dataBill.order, state.dataBill.price.totalPriceFinal);
  ...
  Future<({String? checkPrinters, String? error, String? resultSendPrintData})>
    ...
    if (order == null)
      return (checkPrinters: null, error: null, resultSendPrintData: null);
