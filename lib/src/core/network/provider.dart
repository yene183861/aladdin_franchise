...
import 'reservation/reservation_repo.dart';
...
final FutureProvider<
  ...
  if (orderSelect != null) {
    ...
    for (var tableName in tableSelectRaw) {
      var tableTemp =
          tableUse.firstWhere((element) => element.name == tableName);
...
final FutureProvider<ProductCheckoutResponseData?> productCheckoutProvider =
  ...
  final productRepo =
      await ref.read(orderRepositoryProvider).getProductCheckout(orderSelect);
...
///
/// [[2]] - Danh sách waiter
final FutureProvider<List<List<dynamic>>>
    tableAvailableAndWaiterTransferOrderProvider =
    FutureProvider<List<List<dynamic>>>(
        (FutureProviderRef<List<List<dynamic>>> ref) async {
...
/// Get link đăng ký Zalo OA
final getLinkZaloOAProvider = FutureProvider.autoDispose<String>((ref) async {
  _logEvent('getLinkZaloOAProvider');
  var link = await ref.read(customerRepositoryProvider).getLinkZaloOA();
  return link;
});

/// danh sách bank
// final bankRestaurantProvider =
//     FutureProvider.autoDispose.family<List<UserBankModel>, ApiBankParam>((ref, param) async {
//   final banks = await ref.read(restaurantRepositoryProvider).getBanks(param);
//   return banks;
// });

/// Lấy danh sách phục vụ tại nhà hàng

final waitersProvider =
    FutureProvider.autoDispose<List<WaiterModel>>((ref) async {
  final orderRepo = await ref.read(orderRepositoryProvider).getOrders();
...
final historyOrderProvider =
  ...
  var result = await ref
      .read(orderRepositoryProvider)
      .getHistoryOrder(startDate: startDate, endDate: endDate);
  return result;
});
...
//     FutureProvider.autoDispose<List<CashierModel>>((ref) async {
//   var result = await ref.read(restaurantRepositoryProvider).getCashiers();
//   return result;
// });
