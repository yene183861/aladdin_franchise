...
class OrderRepositoryImpl extends OrderRepository {
  ...
  Future<CreateOrderResponse> createAndUpdateOrder(
    ...
    try {
      if (useDataFake) {
        ...
        if (order.id == 0) {
          final res =
              await kCreateNewOrder(tableId: tableIds, typeOrder: typeOrder);
          if (res.orderId != null) {
            return CreateOrderResponse(
              status: 200,
              orderId: res.orderId,
              message: 'Tạo đơn bàn thành công',
            );
          } else {
            throw res.message ?? 'Lỗi tạo đơn bàn';
          }
        } else {
          final res = await kUpdateTableOrder(
            order: order,
            tableId: tableIds,
            typeOrder: typeOrder,
          );
          if (res.orderId != null) {
            return CreateOrderResponse(
              status: 200,
              orderId: res.orderId,
              message: 'Cập nhật đơn bàn thành công',
            );
          } else {
            throw res.message ?? 'Lỗi cập nhật đơn bàn';
          }
        }
      }
  ...
  Future<ProductCheckoutResponse> getProductCheckout(OrderModel? order) async {
    ...
    var log = ErrorLogModel(
      ...
      api: apiUrl,
      modelInterface: ProductCheckoutResponse.getModelInterface(),
      order: order,
    );
    try {
      if (useDataFake) {
  ...
  Future<void> sendPaymentRating(
      ...
      required ListCustomerRating customerRatings}) async {
